...
## Post-sprint (2026-08-16): sweep predicate is embeddings-only

**The "Not ingested" flip recurred** and survived an API restart. Diagnosis: the sweep predicate gated on `last_ingested_at IS NULL`, but the marker is stamped on **completion** (success or no-text) — so a source with a **stale marker but zero embeddings** (an interrupted re-ingest that deleted the old rows before the write-new-then-swap landed, or a pre-Sprint-73 delete-then-insert interruption) is invisible to the sweep. The Browser's "Ingested" ground truth is embeddings count (`ChunkCount > 0`), so the sweep's candidate set disagreed with the status it was meant to self-heal. The API logs confirmed it: "Ingestion reconciliation: no registered documents missing ingestion" on both restarts while 3 sources sat at zero embeddings with non-null markers.

**Fix:** `GetSourcesMissingIngestionAsync` now targets **any source with zero embeddings, regardless of the marker** — the `last_ingested_at IS NULL` gate is dropped. The candidate set now matches the Browser's ground truth exactly. Trade-off: a checked-and-non-ingestable source (no extractable text) is re-checked each restart — cheap (download + extract, no LLM in the no-text branch) and self-healing if it later becomes extractable. The marker remains as an audit record, not a sweep gate. Tests: Repository 172 (+2 `IngestionReconciliationSweepBindingTests` binding tests that read the repository source so a regression re-gating the sweep on the marker fails the build). The durable job queue (backlog item 1) remains the deferred structural fix — it would prevent the half-state from occurring in the first place.
