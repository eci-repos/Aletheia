...
- **Sprint 60** ("GraphRAG and LazyGraphRAG Quick Wins") is active — see `docs/File 02-Current-Sprint.md` and `docs/sprints/Sprint-60 - GraphRAG and LazyGraphRAG Quick Wins.md`. Its four deliverables (per-request budget, real token accounting + hard deadline, noise-entity filtering, retrieval trace) are implemented, committed, and pushed (`c6c3e48`); the optional Docker smoke test is the next action.
- `Aletheia.Web.UnitTests` has **6 pre-existing failures** (CopilotStateService session-key `v1`/`v2`, `RepositoryApiClientUploadTests` ×4, Wiki mode-buttons) — verified identical on clean HEAD, unrelated to recent sprints; tracked separately. All other suites are green (RAGS 265 / Repository 121 / Integration 8 / Foundation 55 as of Sprint 60).
