...
| 3 | **Startup reconciliation sweep** — detect documents with metadata but no embeddings and auto-queue a repair. | Self-heals interrupted ingestions with zero user action; the "Not ingested" flip becomes self-correcting. | ~1 day | **Implemented** (Sprint 73); post-fix 2026-08-16 — predicate is embeddings-only (the `last_ingested_at IS NULL` gate was dropped after a stale-marker source recurred) |
