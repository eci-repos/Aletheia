...
- **Sprint 62** ("GraphRAG Soft Deadline and Reembed Parity") is **active** — see `docs/File 02-Current-Sprint.md` and `docs/sprints/Sprint-62 - GraphRAG Soft Deadline and Reembed Parity.md`. It promotes backlog items 7 + 8 from `docs/backlog/GraphRAG-LazyGraphRAG-Enhancements.md`, both surfaced by the Sprint 60 Docker smoke test (2026-08-10): item 7 (reembed indexer parity — reembed uses `IndexLightweightAsync` via a new `KnowledgeIndexMode` param on `EnsureIngestedAsync`, default `Full` for repair/chat hydration) and item 8 (GraphRAG soft deadline — a deadline-fires retrieval degrades to best-effort semantic retrieval with trace strategy `semantic-timeout-fallback` instead of HTTP 400).
