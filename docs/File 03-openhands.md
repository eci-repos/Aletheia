...
# Sprint 75 Notes (Activity and Chats Right Rail)

- Active sprint: `docs/sprints/Sprint-75 - Activity and Chats Right Rail.md` (created 2026-08-16). Sprint 74 complete/committed/pushed (`343d9ca` + post-sprint `d8cb518`). Promotes `docs/backlog/Activity-Chats-Right-Rail.md` — the project-owner-directed layout pass: the right-side **Activity** and **Chats** panels "block the view of visual artifacts behind"; owner confirmed **Option A** — the panels collapse into vertical strips with related icons + count badges, and expand to **push** content. Starts **collapsed by default**. **Web-only — no API, backend, or schema changes.**
- **In-flow right rail (item 1)**: `Layout/MainLayout.razor` renders `<ActivityPanel />` + `<ChatsPanel />` inside a `<div class="right-rail">` right column after `<main>` (`.right-rail` in `MainLayout.razor.css`: `display: flex; flex-direction: column; flex: 0 0 auto; height: 100vh; position: sticky; top: 0;`). The panels no longer use `position: fixed` on desktop — each `<aside>` is a flex row (`[shell][toggle strip]`, `align-self: flex-end`), 42px collapsed / `width: 420px; flex: 1 1 auto;` open. `main` is `flex: 1; min-width: 0`, so opening a panel **pushes** content instead of covering it. **When adding a right-edge panel, put it in the `.right-rail` — never `position: fixed` over content.**
- **Collapsed strip = icon + vertical label + count badge (item 2)**: the toggle button keeps its vertical `writing-mode` label ("Activity" / "Chats") and gains an icon span (`activity-toggle-icon icon-activity` / `chats-toggle-icon icon-chats`); the count badges (`activity-count` running count, `chats-count` open conversations) are retained. The `icon-activity` / `icon-chats` `--icon` mask URLs live in `wwwroot/css/app.css` (global, same convention as the NavMenu `nav-icon` set); the panels' scoped `.activity-toggle-icon` / `.chats-toggle-icon` classes apply the mask.
- **Two stacked strips (item 3)**: Activity above, Chats below, each `flex: 0 0 auto` collapsed and `flex: 1 1 auto` open — they split the rail height when both are open; a collapsed strip takes no vertical space. Independent toggles (`isOpen` per panel, both default `false`).
- **Responsive fallback (item 4)**: below `640.98px` the `.right-rail` becomes `position: fixed; top: 0; right: 0; bottom: 0; z-index: 30;` (full-height overlay, the pre-Sprint-75 behavior) and an open panel widens to `calc(100vw - 12px)` — narrow screens are not crushed.
- **Tests + docs (item 5)**: Web **133** (+6) — new `RightRailBindingTests` (MainLayout renders the panels inside a `.right-rail`; panels collapsed by default; `icon-activity`/`icon-chats` + `activity-count`/`chats-count` present; panel CSS in-flow with a `@media (max-width: 640.98px)` overlay fallback; `app.css` defines the two icons). Foundation 55 / Repository 157 / RAGS 361 unchanged; `dotnet build Aletheia.slnx` succeeds (0 errors). Backlog item archived. Residual manual (user-side): `docker compose up -d --build`, then hard-refresh any page (e.g. `/graph`, `/search`) — the strips sit at the right edge as 42px icon strips; opening one pushes content; both start collapsed; on a narrow window (< 641px) the rail returns to an overlay.
- **Out of scope (documented follow-ups)**: changing panel content, the Activity log data flow, job polling, or the Chats conversation restore flow; merging Activity and Chats into a single shared rail or tab control; any API/backend/schema change; changing the left sidebar layout or the Copilot page's own internal three-column grid (`.copilot-layout`).

# Sprint 74 Notes (UI List Scrolling and Collapsible Forms)

- **Complete 2026-08-15** (`343d9ca` + post-sprint `d8cb518`). Sprint file: `docs/sprints/Sprint-74 - UI List Scrolling and Collapsible Forms.md`. Promoted `docs/backlog/UI-List-Scroll-and-Collapsible-Forms.md` — the project-owner-directed UI pass: long lists should scroll inside their own panel (not force the whole panel/page to scroll), the `/lexicon` "Add concept" form should be **collapsed by default** (expand on demand), and Add concept + Unmapped terms belong in a **tab control**; the ideas apply to every page with a similar control layout. **Web-only — no API, backend, or schema changes.**
