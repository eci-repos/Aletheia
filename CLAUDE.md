...
- **Sprint 61** ("Chat Approval Prompt and Admin Settings") is active — see `docs/File 02-Current-Sprint.md` and `docs/sprints/Sprint-61 - Chat Approval Prompt and Admin Settings.md`. It promotes all 5 items from `docs/backlog/Chat-Approval-Prompt-and-Admin-Settings.md`. Items 1-4 are implemented and pushed (`793fc52`): item 1 (modal approval prompt — `PlanPreview` in a centered overlay above the Activity/Chats panels, auto-expand collapsed Execution column on submit), items 2+3+4 (server-side settings foundation `app_settings`/`user_settings` + singleton `SettingsService` + `GET/PUT /api/settings` + `copilot.requireApproval` per-user preference with a "Don't ask again" checkbox + `copilot.requireApproval.force` admin override). Item 5 (admin `/settings` page — My Preferences + Administrator-gated Global Settings card, admin-only NavMenu entry) is **implemented**. Sprint 60 (`c6c3e48`) is complete; its optional Docker smoke test was **run 2026-08-10** (retrieval traces verified end-to-end on LazyGraphRAG + GraphRAG, per-request budgets clean under 5 concurrent LazyGraphRAG retrievals, concurrent GraphRAG hits the 30s hard deadline under LLM saturation, reembed verified working but slow — full `IndexAsync`). The compose topology now env-maps the internal-search gate: `SHOW_INTERNAL_SEARCH=true docker compose up -d api` (default `false`).
