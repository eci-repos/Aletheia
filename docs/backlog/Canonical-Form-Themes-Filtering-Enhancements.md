...
| 5 | **Theme-aware graph retrieval (Phase 2, defer)** — extend theme enforcement to GraphRAG/LazyGraphRAG global/broad paths and community summaries. | Graph modes are currently explicitly out of scope for theme filtering (Sprint 58); a global knowledge-scope widget over graph surfaces is a natural follow-up. | ~1–2 days | **Promoted -> Sprint 64, implemented** (2026-08-11). `sourceIds` scope on graph retrieve/global-search + community filtering; Search Center theme scope now applies to graph modes. |
