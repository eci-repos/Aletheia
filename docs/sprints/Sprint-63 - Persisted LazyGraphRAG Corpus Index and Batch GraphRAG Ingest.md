...
- **Part 1 — corpus persistence across API restart: VERIFIED.** The corpus tables were empty because the 3 registered documents had never been RAGS-ingested (no embeddings either) and the corpus is populated only via LazyGraphRAG ingest (`POST /api/jobs/lazygraphrag/ingest`). Downloaded the 3 registered `.docx` documents, extracted text, enqueued LazyGraphRAG ingest jobs → corpus persisted (3 documents, 3314 terms in `lazygraphrag_corpus_documents` / `lazygraphrag_corpus_terms`). Baseline LazyGraphRAG retrieval returned 6 results (strategy `lazy-semantic`, step `corpus-search`). After `docker compose restart api`, the same query returned the **same 6 results with the same scores (0.692 / 0.69 / 0.688) and the same `corpus-search` step** — proving the corpus loaded from PostgreSQL persistence, not re-discovery.
- **Part 2 — batched (UNWIND) writes + bounded concurrency: VERIFIED via graph state.** Ran the full GraphRAG ingest (`POST /api/graphrag/ingest`) on the largest document (cmp2022, ~42.5k chars) with a fresh source id → HTTP 200 in 774s. Neo4j graph state confirmed the batched writes landed (192 Concept + 58 Technology + 54 Chunk + 21 Document nodes for the fresh source). Direct UNWIND query-log capture was attempted (Neo4j 5.26 container query-log appender quirk — `db.logs.query.enabled=VERBOSE` + `db.logs.query.threshold=0` never wrote the query log) but the writes themselves are confirmed by graph state; bounded LLM concurrency (`MaxLlmConcurrency = 4`) is covered by unit test `IngestAsync_bounded_concurrency_does_not_exceed_limit`. Smoke-source graph nodes were deleted after verification (0 leftover); the 3 corpus documents remain as legitimate data. Neo4j config restored to default (query-log settings removed) and container healthy.
