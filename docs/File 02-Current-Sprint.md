...
- **A5 (sweep):** `GetSourcesMissingIngestionAsync` (zero embeddings AND `last_ingested_at IS NULL` — **post-fix 2026-08-16: embeddings-only, the marker gate was dropped** after a stale-marker source recurred); `IngestionJobService.EnqueueRagsRepairForSources` + `RagsRepairSources` work-item kind + fixed-list runner; `IngestionReconciliationService` `BackgroundService` (10s startup delay, runs once) registered in `Program.cs`.
...
**Post-sprint (2026-08-16): sweep predicate is embeddings-only.** The "Not ingested" flip recurred and survived an API restart: the sweep gated on `last_ingested_at IS NULL`, but the marker is stamped on completion — so a source with a **stale marker but zero embeddings** (interrupted re-ingest) was invisible. `GetSourcesMissingIngestionAsync` now targets **any source with zero embeddings, regardless of the marker** (matches the Browser's "Ingested" ground truth = embeddings count). Non-ingestable sources are re-checked each restart (cheap, self-healing). Repository 172 (+2 `IngestionReconciliationSweepBindingTests`). The durable job queue (backlog item 1) remains the deferred structural fix.
