...
- **Sprint 65** ("Wiki Markdown and HTML View Tabs") is **active** — see `docs/File 02-Current-Sprint.md` and `docs/sprints/Sprint-65 - Wiki Markdown and HTML View Tabs.md`. It promotes `docs/backlog/Wiki-Markdown-HTML-Tabs.md`: Wiki pages (stored as markdown in `WikiPage.Summary`) get a **View / Source** tab control — View renders the summary through a shared `MarkdownRenderer.ToHtml` (extracted from Copilot's former private renderer), Source shows raw md in a `<pre>`; RTF rejected (browsers can't render it inline); ephemeral page state, no wire/API changes.
