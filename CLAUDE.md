...
- **Sprint 66** ("Remove Redundant Metadata Nav Item") is **active** — see `docs/File 02-Current-Sprint.md` and `docs/sprints/Sprint-66 - Remove Redundant Metadata Nav Item.md`. It promotes `docs/backlog/Remove-Redundant-Metadata-Nav-Item.md`: the **Metadata** side-menu item is removed from `NavMenu.razor` (it duplicated Browse — Browse's ✎ Edit action already deep-links to `metadata?fileId=...`); the `/metadata` page/route stay untouched. The "Searching…" hang on the Metadata/Browse search is an API-availability diagnostic, not a code bug (`/api/search` is a plain PostgreSQL metadata query, no LLM).
