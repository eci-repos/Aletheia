...
| RagsController     | \/api/rags/status\     | Retrieval diagnostics: chunk/source counts, uncategorized ingests, recent upload jobs |
| KnowledgeController | `/api/knowledge`      | Themes catalog, uncategorized document list, template re-evaluation |
...
### Knowledge Themes (Sprint 58) and Multi-Theme / Template Status (Sprint 59)

- **Concept**: every canonical template in `docs/doc-templates` declares a knowledge theme **set** on its first line (`Theme: <Theme>`, e.g. `Theme: Analysis`, or `Theme: Analysis, As-Built` for multiple). The themes are persisted on `file_metadata.template_name` / `file_metadata.theme` (`text[]`) / `file_metadata.template_status` at ingestion (idempotent migration `2026-08-07-file-metadata-template-status-themes.sql`; fresh installs get it from `init.sql`). Documents ingested before Sprint 58 fall back to themes derived from the file name via the template registry (safety net only — persisted values win).
- **`template_status`**: `Canonical` when a template matches; `Uncategorized` when none does (a template with no `Theme:` line also resolves to `Uncategorized`). Since Sprint 59, uncategorized documents are still **ingested** (RAGS + knowledge index + graph seed) so a new document kind is never lost; only template-dependent features (document briefs, per-section retrieval, theme) wait for promotion.
- **Uncategorized list + re-evaluation**: `GET /api/knowledge/uncategorized` (authenticated) lists rows whose `template_status` is null or not `Canonical` (file, status, template). `POST /api/knowledge/reevaluate` re-resolves the template for one (`{ "sourceId": "..." }`) or all rows, persists `template_name`/`theme`/`template_status`, and enqueues a document brief for rows that become `Canonical`; it returns `{ evaluated, promoted, uncategorized }`. This is also the one-time backfill for pre-Sprint-58 rows with null derived fields. In the Web UI, the Search Center admin section (with `FeatureFlags:ShowInternalSearch=true`) has **List uncategorized** and **Re-evaluate all** buttons.
- **End users**: the Copilot session picks knowledge themes at session creation (theme picker) and shows them as chips in the session header, editable mid-session. An empty selection ("All themes") means all documents - the pre-Sprint-58 behavior. Search Center has its own shared **Theme scope** filter applied to semantic search (see below).
- **API**: `GET /api/knowledge/themes` (authenticated) returns `[{ theme, documentCount }]`, including themes with zero registered documents so the picker is stable. A document in multiple themes is counted in each.
- **Enforcement**: the selected themes are resolved to registered source ids (`IKnowledgeThemeService`, singleton; match-any semantics) and enforced in every RAGS retrieval path - vector and keyword fallback (`RetrievalRequest.SourceIds`, `source_id = ANY(...)` in PgVectorStore) - and on repository-tool results in the execution engine. A named document outside the session themes returns no results from that document. Search Center applies its shared scope via `GET /api/rags/retrieve?themes=<comma-separated>` (semantic search only).