...
- A Copilot session can be scoped to knowledge themes (Analysis, As-Built, As-Proposed, ...), and Search Center has its own shared **Theme scope**. Themes come from the canonical templates (`docs/doc-templates`, first-line `Theme: ...`, comma-separated for multiple) and are persisted per document at ingestion. A template can declare multiple themes; a document in any selected theme matches.
- **Scope coverage (Sprint 64)**: the Search Center theme scope applies to **semantic** search (`GET /api/rags/retrieve?themes=`) **and** to the internal graph modes — `GET /api/graphrag/retrieve?themes=`, `GET /api/graphrag/global?themes=`, `GET /api/lazygraphrag/retrieve?themes=`, `GET /api/lazygraphrag/global?themes=` (comma-separated theme names). GraphRAG/LazyGraphRAG retrieval filters resolved entities / corpus seed sources to the selected themes and scopes their semantic fallback; global search filters community summaries to communities whose members belong to the selected themes (a scoped global search with no matching communities returns an error). WRAGS search stays curated (theme scope does not apply).
