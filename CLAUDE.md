...
- **Sprint 63** ("Persisted LazyGraphRAG Corpus Index and Batch GraphRAG Ingest") is **active** — see `docs/File 02-Current-Sprint.md` and `docs/sprints/Sprint-63 - Persisted LazyGraphRAG Corpus Index and Batch GraphRAG Ingest.md`. It promotes the last two parked backlog items 2 + 3 from `docs/backlog/GraphRAG-LazyGraphRAG-Enhancements.md`: item 2 (persist the LazyGraphRAG corpus index to PostgreSQL — `ICorpusIndexRepository` → `PostgreSqlCorpusIndexRepository`, `lazygraphrag_corpus_documents`/`lazygraphrag_corpus_terms` tables, `CorpusDiscoveryIndex` loads at startup + persists write-through, best-effort so the in-memory index stays authoritative) and item 3 (batch GraphRAG ingest — `IGraphProvider.CreateNodesAsync`/`CreateRelationshipsAsync`/`UpdateNodesAsync` with default interface impls + `Neo4jGraphProvider` UNWIND writes, bounded-concurrency LLM extraction at `MaxLlmConcurrency = 4`, community re-clustering gated on `!sourceExists`).
