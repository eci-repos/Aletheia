...
**Batched full ingest (Sprint 63):** the full graph-intelligence ingest path (`UploadedContentKnowledgeIndexer.IndexAsync` / `GraphRagService.IngestAsync`, used by repair and chat hydration) is no longer serial N+1. `IGraphProvider` exposes batch methods (`CreateNodesAsync` / `CreateRelationshipsAsync` / `UpdateNodesAsync`, with default interface implementations that fall back to per-item calls) and `Neo4jGraphProvider` implements them with `UNWIND` Cypher grouped by node label / relationship type. Both ingest paths run 4 phases with bounded-concurrency LLM extraction (`MaxLlmConcurrency = 4`): per-chunk entity + relationship discovery, one batched node + relationship write per label/type group, deduped entity summaries, and **gated community re-clustering** — community detection runs only when the source is new to the graph (`!sourceExists`), so re-ingests skip the O(graph) scan.
...
- `IGraphTraversalBudget` limits LLM calls, depth, nodes, relationships, token budget, and execution time. Since Sprint 60 the budget is **per-request**: LazyGraphRAG keeps an injected template and calls `CreatePerRequest()` inside `RetrieveAsync`, GraphRAG constructs one inline (or via an injectable `budgetFactory` for tests), and no singleton registration exists. Token consumption is recorded from real SemanticKernel usage (`TokenUsageHelper` reads `ChatMessageContent.Metadata`); `RecordTokens` records actual consumption even when it breaches the budget so `IsExceeded()` halts traversal. Both `RetrieveAsync` paths enforce a hard deadline via `CancellationTokenSource.CancelAfter(MaxExecutionTime)`; since Sprint 62 the GraphRAG path treats a deadline that fires without caller cancellation as a **soft** signal and degrades to a best-effort plain semantic retrieval (trace strategy `semantic-timeout-fallback`, steps `deadline-exceeded`/`semantic-fallback`) instead of hard-failing. LazyGraphRAG's reembed job uses the lightweight deterministic indexer (`IndexLightweightAsync`) for parity with file uploads — see the ingestion section.
...
- **Corpus index persistence (Sprint 63):** the in-memory `CorpusDiscoveryIndex` (singleton) is the hot path, but since Sprint 63 it is backed by `ICorpusIndexRepository` → `PostgreSqlCorpusIndexRepository` (Dapper, tables `lazygraphrag_corpus_documents` + `lazygraphrag_corpus_terms`). The index loads the persisted corpus at startup and persists write-through on each `IndexAsync`; both are **best-effort** — a load/persist failure logs a warning and never fails ingestion, and the in-memory index stays authoritative. This makes the corpus survive a container restart and a second API instance (candidate selection no longer starts from an empty corpus).
...
- **Indexer mode (Sprint 62)**: `EnsureIngestedAsync` takes a `KnowledgeIndexMode` (`Full` default / `Lightweight`). `Full` runs `UploadedContentKnowledgeIndexer.IndexAsync` (LLM graph intelligence: entity discovery, node summaries, relationship extraction, community detection + summaries); `Lightweight` runs `IndexLightweightAsync` (no LLM — deterministic topic extraction, taxonomy tags, ontology source/topic entities, graph seed nodes with `lazyEnrichmentStatus = "Pending"`), matching file uploads. The reembed job passes `Lightweight` so re-embedding after a provider/dimension change does not re-derive the (lazily produced) graph intelligence; repair and chat hydration keep `Full`.
...
- **Shared theme scope (Phase 1)**: `SearchScopeStateService` (scoped, localStorage key `aletheia.search.scope.v1`) holds a shared theme selection (empty = all themes). Search Center (`Pages/SearchCenter.razor`) renders a theme filter above the search box (chips with per-theme document counts), persists selection into the shared scope, and applies it to **semantic** search via `GET /api/rags/retrieve?themes=` (controller resolves -> `RetrievalRequest.SourceIds`) with a visible "Scoped to N themes" indicator. Copilot keeps its session-scoped `ChatSession.ThemeFilter`; Wiki stays curated. Since Sprint 64 the scope also applies to the graph modes (see below).
...
- **`sourceIds` scope on graph services**: `IGraphRagService.RetrieveAsync`/`GlobalSearchAsync`, `ILazyGraphRagService.RetrieveAsync`/`GlobalSearchAsync`, and `IGlobalGraphSearchService.SearchAsync` take an optional `IReadOnlyList<Guid>? sourceIds = null` (after `cancellationToken`). Null = no scope (pre-Sprint-64 behavior). `GraphThemeScope` (static, `RAGS.Application/GraphRAG`) resolves a node's owning source id (`Properties["sourceId"]` as Guid/string, falling back to `Type == "Source"` → `Id`) and filters nodes/communities to an allowlist.
- **Enforcement points**: `GraphRagService.RetrieveAsync` filters resolved entities and multi-hop expansion nodes to the allowlist and scopes the semantic fallback / entity-expansion `RetrievalRequest`s with `sourceIds`; `LazyGraphRagService.RetrieveAsync` filters the corpus seed sources (`SearchCorpus` results) before entity discovery and scopes fallback / expansion requests; `GlobalGraphSearchService.SearchAsync` builds a node→source map via `IGraphProvider.GetNodesAsync()` and filters communities with **match-any** semantics (a community survives when any member's source is in the allowlist), returning `Failure("No communities in the selected themes.")` when scoped and nothing survives. `GlobalGraphSearchService` ctor gained optional `IGraphProvider? graphProvider = null` (after `kernel`).
