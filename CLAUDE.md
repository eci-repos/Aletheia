...
- **Sprint 69** ("Ingestion Status in the Repository Browser") is **active** (2026-08-13) — `docs/sprints/Sprint-69 - Ingestion Status in Repository Browser.md` promotes `docs/backlog/Ingestion-Status-in-Repository-Browser.md`. The Repository Browser now shows whether an uploaded file actually made it into retrieval: `IVectorStore.GetChunkCountsAsync` (default no-op impl so fakes keep compiling; `PgVectorStore` grouped `COUNT(*)` override) → `SearchController` stamps each `FileMetadata` with `ChunkCount`/`Ingested` (ground truth = embeddings, not job status) → `Browse.razor` Ingestion column with green **Ingested** / amber **Not ingested** badges + failure-mode tooltip. Repository 137 (+3) / Web 79 (+3) / RAGS 302 / Foundation 55 green; build 0 errors. Residual manual: hard-refresh `/browse` — the CMP 2026 RFP row should show **Not ingested**, confirming the failed-job diagnosis.
